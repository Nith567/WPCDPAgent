 ...
 * @param blogContent
 */
async function generateContentSummary(blogContent: string): Promise<string | null> {
  try {
    const prompt = `
 ...
 * @param content
 */
async function uploadContentTo0G(
  content: string,
): Promise<{ rootHash: string; transactionHash: string } | null> {
  try {
    const privateKey = process.env.ZG_PRIVATE_KEY;
  ...
  try {
    ...
    if (!storageResult) {
      return NextResponse.json(
        { message: "Failed to upload content to 0G Storage." },
        { status: 500 },
      );
    }
