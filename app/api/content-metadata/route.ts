...
/**
 * API endpoint to serve content metadata from the filesystem
 *
 * @returns JSON response with content metadata array or error
 */
export async function GET() {
  try {
    ...
    return NextResponse.json({ error: "Failed to read content metadata" }, { status: 500 });
  }
}
